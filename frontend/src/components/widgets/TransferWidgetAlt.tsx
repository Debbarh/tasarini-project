...
const TRANSFER_ALT_SCRIPT_SRC = "https://c91.travelpayouts.com/content?currency=USD&trs=476135&shmarker=686719&powered_by=false&locale=en&mode=default&theme=white&layout=default&promo_id=4770";

interface TransferWidgetAltProps {
  className?: string;
}

export const TransferWidgetAlt = ({ className = "" }: TransferWidgetAltProps) => {
  ...
  useEffect(() => {
    ...
    const appendScript = () => {
      ...
      script.src = TRANSFER_ALT_SCRIPT_SRC;
      script.async = true;

      script.addEventListener("load", triggerInit);
      script.addEventListener("error", () => {
        console.error('❌ Failed to load TravelPayouts transfer (alt) script');
    ...
    const selector = `script[src="${TRANSFER_ALT_SCRIPT_SRC}"]`;
...
export default TransferWidgetAlt;
