...
const TRANSFER_SCRIPT_SRC = "https://tpwdg.com/content?trs=476135&shmarker=686719&locale=en&city=68511&show_header=true&powered_by=false&campaign_id=627&promo_id=8951";

interface TransferWidgetProps {
  className?: string;
}

export const TransferWidget = ({ className = "" }: TransferWidgetProps) => {
  ...
  useEffect(() => {
    ...
    const appendScript = () => {
      ...
      script.src = TRANSFER_SCRIPT_SRC;
      script.async = true;

      script.addEventListener("load", triggerInit);
      script.addEventListener("error", () => {
        console.error('❌ Failed to load TravelPayouts transfer script');
    ...
    const selector = `script[src="${TRANSFER_SCRIPT_SRC}"]`;
