...
const AVAILABLE_MODELS = {
  openai: [
    ...
    { value: 'gpt-4o', label: 'GPT-4o (Puissant et créatif)' },
  ...
  gemini: [
    { value: 'gemini-2.5-flash', label: 'Gemini 2.5 Flash (Recommandé - Rapide et créatif)' },
...
const StoryAIProviderManagement = () => {
  const [providers, setProviders] = useState<StoryAIProviderConfig[]>([]);
  ...
  const fetchProviders = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await storyAIProviderService.list();
      setProviders(data || []);
    } catch (err) {
      console.error("Error fetching Story AI providers", err);
  ...
  const updateProvider = async (id: string, updates: Partial<StoryAIProviderConfig>) => {
    setSavingId(id);
    try {
      const updated = await storyAIProviderService.update(id, updates);
      setProviders((prev) => prev.map((provider) => (provider.id === id ? updated : provider)));
      toast({
        title: "Configuration mise à jour",
        description: "Le modèle d'IA pour les stories a été mis à jour.",
      });
    } catch (err: any) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Configurez les modèles d'IA pour la génération de récits de voyage
            </p>
          </div>
        </div>
      ...
      <CardContent>
          ...
          <div className="mb-4 p-3 bg-primary/5 rounded-lg border border-primary/10">
            ...
            <div className="flex flex-wrap gap-2">
              {providers
                ...
                .map(p => (
                  <Badge key={p.id} variant="default" className="font-medium">
                    {p.display_name} ({p.model_name})
                  </Badge>
                ))}
            </div>
            <p className="text-xs text-muted-foreground mt-2">
              Le premier provider actif (ordre alphabétique) sera utilisé pour générer les stories
            </p>
          </div>
        )}
                ...
                <TableCell className="font-medium">
                  <div className="flex items-center gap-2">
                    ...
                    {provider.provider === 'perplexity' && (
                      <Badge variant="outline" className="text-xs">Faits réels</Badge>
                    )}
                  </div>
                </TableCell>
                <TableCell>
        ...
        <div className="mt-6 p-4 bg-muted/50 rounded-lg space-y-2">
          ...
          <ul className="text-sm text-muted-foreground space-y-1 list-disc list-inside">
            <li><strong>Modèle :</strong> Sélectionnez le modèle d'IA. Les modèles "Flash" sont plus rapides, les "Pro" plus créatifs.</li>
            <li><strong>Température :</strong> Contrôle la créativité (0.5 = structuré, 1 = très créatif). Recommandé pour stories : 0.75</li>
            <li><strong>Perplexity :</strong> Excellent pour des récits riches en faits réels et informations vérifiées.</li>
...
export default StoryAIProviderManagement;
