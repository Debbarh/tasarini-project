...
const TOURS_ALT_SCRIPT_SRC = "https://tpwdg.com/content?currency=USD&trs=476135&shmarker=686719&product=&language=en&layout=horizontal&powered_by=false&campaign_id=89&promo_id=3948";

interface ToursWidgetAltProps {
  className?: string;
}

export const ToursWidgetAlt = ({ className = "" }: ToursWidgetAltProps) => {
  ...
  useEffect(() => {
    ...
    const appendScript = () => {
      ...
      script.src = TOURS_ALT_SCRIPT_SRC;
      script.async = true;

      script.addEventListener("load", triggerInit);
      script.addEventListener("error", () => {
        console.error('❌ Failed to load TravelPayouts tours (alt) script');
    ...
    const selector = `script[src="${TOURS_ALT_SCRIPT_SRC}"]`;
